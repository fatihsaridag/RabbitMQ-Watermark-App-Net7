...
namespace RabbitmqApp.MVC.BackgroundServices
{
    public class ImageWatermarkProcessBackgroundService : BackgroundService
    {
        ...
        private  Task Cunsomer_Received(object sender, BasicDeliverEventArgs @event)
        {
            ...
            try
            {
                ...
                var font = new Font(FontFamily.GenericMonospace, 60, FontStyle.Bold, GraphicsUnit.Pixel);
                var textSize = graphic.MeasureString(siteName, font);
                var color = Color.FromArgb(128, 255, 255, 255); 
                var brush = new SolidBrush(color);
                var position = new Point(img.Width - ((int)textSize.Width + 5), img.Height - (int)textSize.Height + 5);
